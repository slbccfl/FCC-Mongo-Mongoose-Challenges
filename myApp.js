...
var createAndSavePerson = function(done) {
  var bob = new Person({
    name: 'Bob',
    age: 32,
    favoriteFoods: ["Lasagna", "Bananas", "Potato Bread"]
  })
...
// 'arrayOfPeople'.
var arrayOfPeople = [
      {name: 'Tom', age: 12, favoriteFoods: ['hotdogs','pizza']},
      {name: 'Dick', age: 13, favoriteFoods: ['hamburgers','pizza']},
      {name: 'Herry', age: 14, favoriteFoods: ['catfish','pizza']}
      ]
...
var personId = findOneByFood.field_id;
...
var findAndUpdate = function(personName, done) {
  var ageToSet = 20;
  Person.findOne({name: personName}, function(err, personData) {
    if (err) return done(err);
    personData.age = ageToSet;
    personData.save(function(err, data) {
      if (err) return done(err);
      done(null, data);
    })
